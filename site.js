...
class SimpleBlog {
  ...
  async loadRandomAssetImages() {
    ...
    // Prefer live repo listing so deleted images are never referenced
    const liveByPath = new Map();
    try {
      // 1) Try GitHub Tree API (recursive) for comprehensive list (handles nested folders)
      ...
      if (treeResp.ok) {
        ...
        data.tree
          .filter(it => it.path.startsWith('assets/images/') && exts.some(ext => it.path.toLowerCase().endsWith(ext)))
          .forEach(it => {
            const name = it.path.split('/').pop();
            const lower = (name || '').toLowerCase();
            const ext = exts.find(ext => lower.endsWith(ext));
            if (ext) {
              liveByPath.set(it.path, {
                name,
    ...
    try {
      // 2) Also try simple contents API for top-level files in assets/images
      ...
      if (resp.ok) {
        ...
        list.forEach(item => {
          ...
          if (item.type === 'file' && exts.some(ext => lower.endsWith(ext))) {
            const ext = exts.find(ext => lower.endsWith(ext));
            const path = `assets/images/${item.name}`;
            if (ext && !liveByPath.has(path)) {
              liveByPath.set(path, {
                name: item.name,
                url: encodeURI(`https://raw.githubusercontent.com/pigeonPious/page/main/${path}?_cb=${cacheBust}`),
                type: ext
              });
            }
          }
    ...
    let images = Array.from(liveByPath.values());

    // 3) If live listing failed/empty, fall back to static index (may be stale)
    if (images.length === 0) {
      try {
          ...
          if (Array.isArray(data.images)) {
            images = data.images
              ...
              .map(p => {
                ...
                const lower = (name || '').toLowerCase();
                const ext = exts.find(ext => lower.endsWith(ext));
                return { name, url: encodeURI(`https://raw.githubusercontent.com/pigeonPious/page/main/${p}?_cb=${cacheBust}`), type: ext };
              });
          }
        }
      } catch {}
    ...
    // 4) As a last resort, try directory HTML scrape (CORS proxy)
    if (images.length === 0) {
      try {
        const corsProxy = 'https://corsproxy.io/?';
        ...
        if (dirResp.ok) {
          ...
          images = matches
            ...
            .map(href => {
              ...
              const lower = (name || '').toLowerCase();
              const ext = exts.find(ext => lower.endsWith(ext));
              return { name, url: encodeURI(`https://raw.githubusercontent.com/pigeonPious/page/main/assets/images/${name}?_cb=${cacheBust}`), type: ext };
            });
        }
      } catch {}
    }
  ...
  displayMedia(placeholder, mediaUrl, mediaName, mediaType) {
    ...
    if (isVideo) {
      ...
      // Let CSS handle styling - only set cursor for click interaction
